<?php
    if (isset($_POST['save'])) {
        $query = "INSERT INTO produk SET nama_produk ='".$_POST['nama_produk']."',  kode = '".$_POST['kode_produk']."' , kategori = ".$_POST['kategori']." , detail ='".$_POST['detail_produk']."' , merek = ".$_POST['merek']." , harga = ".$_POST['harga']." , stok =".$_POST['stok']." , best_seller =".$_POST['best']."";
        $stat = mysqli_query($l , $query) or die(mysqli_error($l)); 
        
        if ($stat) {
        $stat2 = mysqli_query($l , "SELECT * FROM produk where kode ='".$_POST['kode_produk']."' ");
        $data2=mysqli_fetch_array($stat2);
          ...
          if(in_array($ekstensi, $ekstensi_diperbolehkan) === true){
            if($ukuran < 10044070){      
              ...
              $query = "UPDATE produk set foto = '".$nama."' where kode ='".$_POST['kode_produk']."'";
              $stat=mysqli_query($l, $query) OR DIE(mysqli_error($l));
              if($stat){
               ?>
               <?php
        ...
        <script type="text/javascript">
            ...
            document.location = 'index2.php?page=tambahtagproduk&idx=<?php echo $data2['id_produk'] ?>'
        </script>
<?php
        } else {
?>
...
<table align="center" style="margin-top: 10px ; width:100%; font-size: 15px; padding: 20px;">
            <tbody>
              <tr>
                <td width="40%">
                    <input style="width: 100%" type="text" name="nama_produk" class="input-data" /><br />
                    <label style="font-size:20px"><b>Nama Produk</b></label><br /><br />

                    <input style="width: 100%" name="kode_produk" maxlength="5"  class="input-data"/><br />
                    ...
                    <textarea style="width: 100%; resize: none;" id="edittext" name="detail_produk" class="input-data" rows="8"></textarea> <br />
                ...
                <td width="40%" valign="top">
                    <select name="merek" title="Pilih Merek" class="input-data-ket">
                                    ...
                                    $sku = mysqli_query($l , $qnya);
                                        ...
                                        while ($dat = mysqli_fetch_array($sku)) {
                                    ?>
                                        <option value="<?php echo $dat['id_merek']?>">
                    ...
                    <input style="width: 100%" name="harga" type="number" class="input-data"/><br />
                    <label style="font-size:20px"><b>Harga Produk</b></label><br /><br />

                    <input style="width: 100%" name="stok" type="number" class="input-data" /><br />
                    <label style="font-size:20px"><b>Stok Produk</b></label><br /><br />
                    
                    <p>Best Seller ?</p>
                      <p>
                      <input class="w3-radio" type="radio" name="best" value="1" checked>
                      <label class="w3-validate">Ya</label>
                      <input class="w3-radio" type="radio" name="best" value="0">
...
<input class="btn-tm" style="padding-left: 50px; padding-right: 50px; font-size: 20px" type="submit" name="save" value="Tambah Produk" />
</form>